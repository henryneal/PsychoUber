...
def get_on_duty_taxis(self):
    on_duty_taxis = []
    for taxi in self._taxis:
        if taxi.onDuty:
            on_duty_taxis.append(taxi)
    return on_duty_taxis
...
class Dispatcher:
    ...
    def fareBid(self, origin, taxi):
        averagebalance = sum(list(map(lambda taxi: taxi._account, self._taxis))) / len(self._taxis)
        avbalance20 = (averagebalance * 0.2)
        if taxi._account <= avbalance20:
            return
    ...
    def _costFare(self, fare):
          ...
          if timeToDestination <= 5:
             return (5+timeToDestination)/0.9

          if timeToDestination <= 25:
             return (10+timeToDestination)/0.9

          if timeToDestination <= 50:
             return 100
                ...
                for taxiIdx in self._fareBoard[origin][destination][time].bidders:
                    if len(self._taxis) > taxiIdx:
                        ...
                        if bidderNode is not None:
                            ...
                            if self._taxis[taxiIdx]._account < avbalance20:
                ...
                for taxiIdx in self._fareBoard[origin][destination][time].bidders:
                    if len(self._taxis) > taxiIdx:
                        ...
                        if bidderNode is not None:
                            ...
                            if len(get_on_duty_taxis(self)) <= 1:
